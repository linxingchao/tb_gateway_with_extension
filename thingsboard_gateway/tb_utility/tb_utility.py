...
class TBUtility:
    ...
    def check_and_import(extension_type, module_name):
        log.info("type:%s,module:%s" % (extension_type,module_name))
        log.info(extension_type + module_name)
        if TBUtility.loaded_extensions.get(extension_type + module_name) is None:
            ...
            try:
                for extension_path in extensions_paths:
                    log.info(extension_type + module_name)
                    log.info(TBUtility.loaded_extensions.get(extension_type + module_name))
...
if __name__ == '__main__':
    tb = TBUtility()
    tb.check_and_import('s7','S7Connector')
    tb2 = TBUtility()
    tb2.check_and_import('modbus','ModbusConnector')
