...
from thingsboard_gateway.connectors.s7.s7_converter import S7Converter, log


class BytesS7UplinkConverter(S7Converter):
    def __init__(self, config):
        self.__datatypes = {
            "timeseries": "telemetry",
            "attributes": "attributes"
        }
        self.__result = {"deviceName": config.get("deviceName", "S7Device %s" % (str(config["unitId"]))),
                ...
                try:
                   ...
                   # word_endian_order = Endian.Little if word_order.upper() == "LITTLE" else Endian.Big
                    ...
                    if response is not None:
                        if configuration["registerType"] in ['I', 'V','Q','M']:
                            result = response
                            #log.info(result)
                            decoded_data = result
                            ...
                            #     decoded_data = result[:configuration.get("wordsCount", configuration.get("registersCount", configuration.get("registerCount", 1)))]
