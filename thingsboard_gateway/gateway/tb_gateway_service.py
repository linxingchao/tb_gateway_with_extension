        ...
        if self.__config.get("connectors"):
            for connector in self.__config['connectors']:
                try:
                    ...
                    classtype = self._default_connectors.get(connector["type"], connector.get("class"))
                ...
                for config in connector_config["config"]:
                    ...
                    try:
                        if connector_config["config"][config] is not None:
                            #log.info(connector_config["config"][config])
                            log.info(self._implemented_connectors)
            ...
            for device in devices_data_in_event_pack:
                if devices_data_in_event_pack[device].get("attributes"):
                    if device == self.name or device == "currentThingsBoardGateway":
                        ...
                        #self._published_events.put(self.tb_client.client.send_attributes(device, devices_data_in_event_pack[device]["attributes"]))
                ...
                if devices_data_in_event_pack[device].get("telemetry"):
                    if device == self.name or device == "currentThingsBoardGateway":
                        ...
                        #self._published_events.put(self.tb_client.client.send_telemetry(device, devices_data_in_event_pack[device]["telemetry"]))
